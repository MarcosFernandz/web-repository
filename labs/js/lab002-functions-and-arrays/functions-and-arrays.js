...
const sum =  (matrizMixta)  =>  {
  
  if( matrizMixta.length ===  0)  return  0

  
  let suma =  0;

  for(let i=  0; i < matrizMixta.length; i++)  {
    let elemento =matrizMixta[i];
    if (typeof elemento  ===  'string')  {
      sumTotalDeMatrizCompleja +=elemento.length;
    } else if ( typeof elemento===true) {
      sumTotalDeMatrizCompleja+=1; 
    } else if (typeof elemento==='object'  && elemento!==null){
      throw new Error("No aceptamos objetos dentro  de esta operacion....");
    }else {
      sumTotalDeMatrizCompleja+=elemento;
    }
  }
return  sumTotalDeMatrizCompleja;
...
function doesWordExist(arrayOfWords, palabraABuscar) {
  if {arrayOfWords.includes(palabraABuscar)}  {
